...
void Slam::registerSubscribers() {
    ...
    sync_wheelOdom_laser_sub_ = new message_filters::Synchronizer<LaserWheelSyncPolicy>(
    ...
    sub_opt_topic_ = node_->create_subscription<std_msgs::msg::String>(
        "opt_topic", 10, std::bind(&Slam::optTopicCallback, this, std::placeholders::_1));
 ...
 */
void Slam::laserWheelOdomSyncCallback(const sensor_msgs::msg::LaserScan::ConstPtr& laser_msg,
                                      const nav_msgs::msg::Odometry::ConstPtr& wheel_odom_msg) {
    std::tuple<bool, unsigned int, pcl::PointCloud<pcl::PointXY>::Ptr> odom_res =
        odom_.odomDealWithInputMessage(laser_msg, wheel_odom_msg);
    ...
    if (is_new_key_frame == true) {
        ...
        pose_graph_.addSE2Edge(key_frame_id - 1,
                               key_frame_id,
        ...
        if (loop_id != -1) { /* Find the loop closure by the scene context */
            bool isPossibleLoop = scan_context_manger_.isPossibleLoop(odom_.key_frames_buffer_[key_frame_id]->getPose(),
                                                                      odom_.key_frames_buffer_[loop_id]->getPose());
            if (!isPossibleLoop) {
                return;
            }
            RCLCPP_INFO(node_->get_logger(), "key_frame_id: %d, loop_id: %d", key_frame_id, loop_id);
            visualization_.publishLineOfTwoPoses(odom_.key_frames_buffer_[key_frame_id]->getPose(),
                                                 odom_.key_frames_buffer_[loop_id]->getPose(),
                                                 opt_path_frame_,
                                                 10000);
            ...
            auto prior_guess = odom_.key_frames_buffer_[loop_id]->getPose().inverse() *
                               odom_.key_frames_buffer_[key_frame_id]->getPose();
            MatrixSE2 loop_est = odom_.icpPointMatch(odom_.key_frames_buffer_[loop_id]->getCloud(),
