...
void Slam::init() {
    RCLCPP_INFO(node_->get_logger(),
                "Slam: All other modules initialized successfully, begin to "
                "initialize the Slam node module");
    declareParameters();
    loadParameters();
    advertisePublishers();
    registerSubscribers();
...
void Slam::declareParameters() {
    node_->declare_parameter<std::string>("sub_wheel_odom_topic_",
                                          "wheel_odom");
    ...
    node_->declare_parameter<std::string>("res_point_cloud_topic_",
                                          "res_point_cloud");
    node_->declare_parameter<bool>("save_g2o_file_", false);
    node_->declare_parameter<int>("opt_graph_iter_", 15);
    node_->declare_parameter<std::vector<double>>("odom_info_vec",
                                                  std::vector<double>{});
    node_->declare_parameter<std::vector<double>>("loop_info_vec",
                                                  std::vector<double>{});
...
void Slam::loadParameters() {
    auto param_loop_inf_vec =
        node_->get_parameter("loop_info_vec").as_double_array();
    auto param_laser_odom_inf_vec =
        node_->get_parameter("odom_info_vec").as_double_array();
    ...
    // print the matrix
    std::cout << "laser_odom_inf_matrix_ = " << std::endl
              << laser_odom_inf_matrix_ << std::endl;
    std::cout << "loop_inf_matrix_ = " << std::endl
              << loop_inf_matrix_ << std::endl;
...
void Slam::advertisePublishers() {
    opt_path_pub_ =
        node_->create_publisher<nav_msgs::msg::Path>(pub_opt_path_topic_, 10);
    opt_pose_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>(
        pub_opt_pose_topic_, 10);
    res_point_cloud_pub_ =
        node_->create_publisher<sensor_msgs::msg::PointCloud2>(
            res_point_cloud_topic_, 10);
    RCLCPP_INFO(node_->get_logger(), "Slam: advertise all publishers");
    return;
}

void Slam::registerSubscribers() {
    laser_sub_ = new message_filters::Subscriber<sensor_msgs::msg::LaserScan>(
        node_, sub_laser_topic_);
    wheel_odom_sub_ = new message_filters::Subscriber<nav_msgs::msg::Odometry>(
        node_, sub_wheel_odom_topic_);
    sync_wheelOdom_laser_sub_ =
        new message_filters::Synchronizer<LaserWheelSyncPolicy>(
            LaserWheelSyncPolicy(100), *laser_sub_, *wheel_odom_sub_);
    sync_wheelOdom_laser_sub_->registerCallback(
        boost::bind(&Slam::laserWheelOdomSyncCallback, this, _1, _2));
    sub_opt_topic_ = node_->create_subscription<std_msgs::msg::String>(
        "opt_topic",
        10,
        std::bind(&Slam::optTopicCallback, this, std::placeholders::_1));
 ...
 */
void Slam::laserWheelOdomSyncCallback(
    ...
    std::tuple<bool, unsigned int, pcl::PointCloud<pcl::PointXY>::Ptr>
        odom_res = odom_.odomDealWithInputMessage(laser_msg, wheel_odom_msg);
    ...
    if (is_new_key_frame == true) {
        ...
        pose_graph_.addSE2Edge(
            key_frame_id - 1,
            key_frame_id,
            odom_.key_frames_buffer_.back()->getRelativeMeasure(),
            laser_odom_inf_matrix_);

        std::pair<int, double> sc_res = scan_context_manger_.addNewFrame(
            key_frame_id, std::get<2>(odom_res));
        unsigned int loop_id = sc_res.first;
        double rot_angle = sc_res.second;
        if (loop_id != -1) { /* Find the loop closure by the scene context */
            bool isPossibleLoop = scan_context_manger_.isPossibleLoop(
                odom_.key_frames_buffer_[key_frame_id]->getPose(),
                odom_.key_frames_buffer_[loop_id]->getPose());
            if (!isPossibleLoop) {
                return;
            }
            RCLCPP_INFO(node_->get_logger(),
                        "key_frame_id: %d, loop_id: %d",
                        key_frame_id,
                        loop_id);
            visualization_.publishLineOfTwoPoses(
                odom_.key_frames_buffer_[key_frame_id]->getPose(),
                odom_.key_frames_buffer_[loop_id]->getPose(),
                opt_path_frame_,
                10000);
            /* The `rot_angle` means rotate the `key_frame_id` to the
             * `loop_id`*/
            // use the odometry measurement as the prior guess
            auto prior_guess =
                odom_.key_frames_buffer_[loop_id]->getPose().inverse() *
                odom_.key_frames_buffer_[key_frame_id]->getPose();
            MatrixSE2 loop_est = odom_.icpPointMatch(
                odom_.key_frames_buffer_[loop_id]->getCloud(),
                odom_.key_frames_buffer_[key_frame_id]->getCloud(),
                prior_guess);
            std::cout << "prior_guess = " << std::endl
                      << prior_guess.matrix() << std::endl;
            pose_graph_.addSE2Edge(
                loop_id, key_frame_id, loop_est, loop_inf_matrix_);
            // save the data of the point clouds
            if (save_g2o_file_) {
                std::string pt_loop =
                    "/home/shuo/tmp/id" + std::to_string(loop_id) + ".pcd";
                // convert 2d point cloud to 3d
                pcl::PointCloud<pcl::PointXYZ>::Ptr loop_cloud(
                ...
                std::string pt_key =
                    "/home/shuo/tmp/id" + std::to_string(key_frame_id) + ".pcd";
...
bool Slam::optTopicCallback(const std_msgs::msg::String::SharedPtr msg) {
    ...
    RCLCPP_INFO(node_->get_logger(),
                "The number of optimized pose: %d",
                pose_graph_.optimizer_->vertices().size());
    pcl::PointCloud<pcl::PointXYZ>::Ptr merged(
        new pcl::PointCloud<pcl::PointXYZ>());
    nav_msgs::msg::Path path;
    for (unsigned int i = 0; i < pose_graph_.optimizer_->vertices().size();
         ++i) {
        ...
        pose_msg.pose.orientation =
            tf2::toMsg(tf2::Quaternion(tf2::Vector3(0, 0, 1), orientation));
        // need to wait some time?
        path.poses.push_back(pose_msg);

        pcl::PointCloud<pcl::PointXYZ>::Ptr tmp_3d(
            new pcl::PointCloud<pcl::PointXYZ>());  // Create map
        pcl::PointCloud<pcl::PointXYZ>::Ptr key_frame_3d(
            new pcl::PointCloud<pcl::PointXYZ>());
        pcl::copyPointCloud(*odom_.key_frames_buffer_[i]->getCloud(),
                            *key_frame_3d);
        // pose_msg to matrix
