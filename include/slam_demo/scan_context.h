...
class ScanContextManger {
   ...
   public:
    ...
    // The user API
    std::pair<int, double> addNewFrame(const unsigned int id, const pcl::PointCloud<pcl::PointXY>::Ptr cloud);
   ...
   private:
    ...
    const unsigned int kSkipLaestFrames = 25;  // To detect the loop closure, we skip latest `kSkipLaestFrames`
                                               // frames
    ...
    const double kSC_DIST_THRES = 0.5;  // The threshold of the distance between two scan contexts (get
    ...
    RingKeyVecOfVecs tmp_ring_key_search_base_;  // A temporary variable to store the ring
                                                 // keys for the KDTree
