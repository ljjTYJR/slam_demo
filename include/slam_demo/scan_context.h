...
#include <utility>
#include <vector>
#include "KD_tree_vector_of_vectors_adaptor.h"
...
class ScanContextManger {
    typedef struct POINT_CLOUD_DSC {
        ...
        std::vector<float>
            ring_key;  // The ring key should be one-dimensional vector
        // pcl::PointCloud<pcl::PointXY>::Ptr cloud;    //TODO: decide whether
        // to store the cloud, in the odometer or here
    } POINT_CLOUD_DSC_;
   ...
   public:
    ...
    // The user API
    std::pair<int, double> addNewFrame(
   ...
   private:
    ...
    Eigen::MatrixXf generateScanContext(
        const pcl::PointCloud<pcl::PointXY>::Ptr cloud);
    ...
    std::pair<double, int> scanContextAlignment(
        Eigen::MatrixXf& q_scan_context,
        Eigen::MatrixXf& ref_scan_context);
    Eigen::MatrixXf scanContextMatShift(Eigen::MatrixXf& scan_context,
                                        int shift);
    std::pair<int, double> detectLoopClosure(const POINT_CLOUD_DSC_& cur_dsc);
    double distDirectSC(Eigen::MatrixXf& q_scan_context,
                        Eigen::MatrixXf& ref_scan_context);
    ...
    const unsigned int kSkipLaestFrames =
        25;  // To detect the loop closure, we skip latest `kSkipLaestFrames`
             // frames
    const unsigned int kNumRingKeyQuery =
        10;  // The number of key frames(ring key) to query
    const double kLoopDistThres =
        0.3;  // The maximum distance between two possbile poses
    ...
    const double kSC_DIST_THRES =
        0.5;  // The threshold of the distance between two scan contexts (get
    ...
    RingKeyVecOfVecs
        tmp_ring_key_search_base_;  // A temporary variable to store the ring
                                    // keys for the KDTree
